...
def  main():
  ...
  installer = "\installer\IPlugLinkedParams Installer.exe"
   
  if demo:
    installer = "\installer\IPlugLinkedParams Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugLinkedParams manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugLinkedParams-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugLinkedParams-v" + FULLVERSIONSTR + "-win-demo.zip"
